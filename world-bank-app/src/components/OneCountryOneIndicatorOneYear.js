...
export default function OneCountryOneIndicatorOneYear(props) {
	...
	useEffect(() => {
		(async () => {
			setData(
				await network.getCountryIndicatorYearInfo(props.country, props.indicator, props.year)
			);
		})();
	}, []);
	...
	return (
		<Container className="py-4">
			...
			<div className="mb-4 bg-light rounded-3">{data ? checkResponse() : <p>loading</p>}</div>
			<Container>
